function dailyqa = LoadDailyQA(path, name, numberOfProjections, openRows, ...
    mvctRows, shiftGold)
% LoadDailyQA parses a TomoTherapy Transit Dose DICOM RT object or patient 
% archive XML file for procedure return data and derives various 
% calibration parameters.  See below for more details on the parameters
% returned.
%
...
if isempty(s) 
    ...
    if exist('Event', 'file') == 2
        Event(sprintf('Number of MVCT detector channels set to %i', ...
            mvctRows));
    end
    rows = mvctRows;
...
% Else .xml was found, so use patient archive to load daily QA result
    ...
    % Report success
    if exist('Event', 'file') == 2
        Event(sprintf(['%i projections successfull loaded across %i ', ...
            'channels'], numberOfProjections, rows));
    end
end
...
if size(peaks,1) == 31
    if exist('Event', 'file') == 2
        Event(['Only 31 odd leaf peaks found, assuming 32nd is ', ...
            'last channel']);
    end
    peaks(32) = rows;
end
...
if size(peaks,1) == 31
    if exist('Event', 'file') == 2
        Event(['Only 31 even leaf peaks found, assuming 32nd is first ', ...
            'channel']);
    end
    peaks(32) = 1;
end
